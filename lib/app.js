...
app.get('/authors', async(req, res) => {
  try {
    const data = await client.query(`
      SELECT authors.author_name, authors.id, author_origin.born, authors.published_books, authors.living 
      from authors
      JOIN author_origin on author_origin.id = authors.born_id
      `);
...
app.get('/authors_origins', async(req, res) => {
  try {
    const data = await client.query(`
      SELECT *
      from authors_origin
